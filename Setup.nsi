; Installer for Open Teradata Viewer 0.32b
;
...
!define PRODUCT_VERSION "0.32b"
!define PRODUCT_PUBLISHER "Open Source Community"
!define PRODUCT_WEB_SITE "http://openteradata.sourceforge.net"
!define PRODUCT_DIR_REGKEY "Software\Microsoft\Windows\CurrentVersion\App Paths\open_teradata_viewer_0.32b.exe"
...
; Finish page
!define MUI_FINISHPAGE_RUN "$INSTDIR\open_teradata_viewer_0.32b.exe"
...
OutFile "open_teradata_viewer_0.32b_setup.exe"
...
Section "Required files" SEC01
  ...
  File "open_teradata_viewer_0.32b.exe"
  File "readme.txt"
  File "terajdbc4.jar"
...
; Shortcuts
  ...
  CreateShortCut "$SMPROGRAMS\$ICONS_GROUP\Open Teradata Viewer.lnk" "$INSTDIR\open_teradata_viewer_0.32b.exe"
  CreateShortCut "$DESKTOP\Open Teradata Viewer.lnk" "$INSTDIR\open_teradata_viewer_0.32b.exe"
  CreateShortCut "$QUICKLAUNCH\Open Teradata Viewer.lnk" "$INSTDIR\open_teradata_viewer_0.32b.exe" "" "$INSTDIR\open_teradata_viewer_0.32b.exe" 0
...
Section -Post
  ...
  WriteRegStr HKLM "${PRODUCT_DIR_REGKEY}" "" "$INSTDIR\open_teradata_viewer_0.32b.exe"
  WriteRegStr ${PRODUCT_UNINST_ROOT_KEY} "${PRODUCT_UNINST_KEY}" "DisplayName" "$(^Name)"
  WriteRegStr ${PRODUCT_UNINST_ROOT_KEY} "${PRODUCT_UNINST_KEY}" "UninstallString" "$INSTDIR\uninst.exe"
  WriteRegStr ${PRODUCT_UNINST_ROOT_KEY} "${PRODUCT_UNINST_KEY}" "DisplayIcon" "$INSTDIR\open_teradata_viewer_0.32b.exe"
...
Section Uninstall
  ...
  Delete "$INSTDIR\open_teradata_viewer_0.32b.exe"
